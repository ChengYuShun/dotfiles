  ...
  (let ((indentation (make-string (current-indentation) ? )))
    (cond
     ((eq evil-state 'visual)
      ...
      (insert indentation)
      (insert end)
      (if (not (equal (buffer-end 1) (point)))
          (forward-char)
        (insert "\n")
        (insert indentation))
      (evil-normal-state 1))
     (t (save-excursion
          ...
          (save-excursion
            ...
            (insert indentation)
            (insert end)
            (if (and (eql (point) (line-end-position))
                     (not (eql (point) (buffer-end 1))))
                (forward-char)
              (insert "\n")
              (insert indentation))
...
(dolist (pair '(("C-s" . nil)
                ...
                ("C-s m f r k" . cys/shorthands-latex-mathfrak)
                ("C-s m s c r" . cys/shorthands-latex-mathscr)
...
(defun cys/shorthands-latex-abs ()
  (interactive)
  (when (cys/shorthands-insert-brackets "\\left| " 'tab " \\right|" 'tab)
    (cys/shorthands-tab)
    (evil-insert-state 1)))

(defun cys/shorthands-latex-norm ()
  (interactive)
  (when (cys/shorthands-insert-brackets "\\left\\Vert " 'tab " \\right\\Vert" 'tab)
    (cys/shorthands-tab)
    (evil-insert-state 1)))
...
(defun cys/shorthands-latex-mathfrak ()
  (interactive)
  (cys/shorthands-latex-apply-macro "mathfrak"))
(defun cys/shorthands-latex-mathscr ()
  (interactive)
  (cys/shorthands-latex-apply-macro "mathscr"))
